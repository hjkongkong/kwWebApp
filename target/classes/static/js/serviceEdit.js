...
$(function() {
	$("#EditButton").click(function() {
		...
		var funcNametoSend =  document.getElementById("getF_name").value;
		...
		if (nullCheck > 0) {
			...
			var link = "/deletefunc/"+funcNametoSend;
			$.ajax({ //ajax1
						...
						$.ajax({ //ajax2
							...
							success : function(result) {
								...
								if (res.success == "true") {
									...
									alert("함수 이름이 중복됩니다. 다른 이름의 함수를 생성해주세요."); //alert for duplication
								}else{alert("result fail - ajax2");}
							},
							failure : function() {
								alert("req_fail");
							},
